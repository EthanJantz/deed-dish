...
async function loadEntityMapping(): Promise<void> {
    ...
    try {
        ...
        console.log(`Loaded entity mapping with ${Object.keys(entityFilesMapping).length} entries`);
    } catch (error) {
...
async function loadEntityData(granteeName: string): Promise<EntityData | null> {
    ...
    try {
        ...
        console.error(`Error loading entity data for ${granteeName} (${filename}):`, error);
        return null;
...
async function checkGranteeExists(granteeName: string): Promise<boolean> {
    ...
    try {
        const response = await fetch(`${ENTITY_API_PATH}${filename}`, { method: 'HEAD' });
        return response.ok;
    } catch {
        return false;
    }
...
async function renderDocuments(pin: string, addresses: string[], documents: Document[]) {
    ...
    const docResults = await Promise.all(sortedDocs.map(async (doc) => {
        ...
        const granteesText = granteeLinks.length > 0 ? ` to ${granteeLinks.join(', ')}` : '';
    ...
    granteeLinks.forEach(link => {
        link.addEventListener('click', async (e) => {
            e.preventDefault();
            const granteeName = (e.target as HTMLElement).getAttribute('data-grantee');
            if (granteeName) {
                console.log(`Grantee link clicked: ${granteeName}, checking if function exists:`, typeof highlightGranteeParcels);
                try {
...
async function highlightGranteeParcels(granteeName: string, originPin: string) {
    try {
        ...
        console.log(`Highlighting ${entityData.ASSOCIATED_PINS.length} parcels for ${granteeName}`);
        
        // Highlight grantee parcels (orange)
        map.setFilter('parcels-highlight-grantee', ['in', 'name', ...entityData.ASSOCIATED_PINS]);
        ...
        if (granteeFeatures.length > 0) {
            ...
            if (!bounds.isEmpty()) {
                ...
                console.log(`Zoomed to fit ${granteeFeatures.length} highlighted parcels`);
            }
        } else {
        ...
        console.error(`Error highlighting grantee parcels for ${granteeName}:`, error);
    }
}
